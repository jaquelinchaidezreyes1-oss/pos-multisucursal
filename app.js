...
/* ── ACCESO PRIVADO DIRECTIVO (MONITOR EN VIVO & CIERRE DE DÍA) ── */
    ...
    async function loadAccounting(silent = false) {
        ...
        c.innerHTML = `
        <!-- ══ ACUMULADO SEMANAL HISTÓRICO DE LAS SUCURSALES (DIRECTORES) ══ -->
        ${(() => {
            // 1. Agrupar todas las ventas activas por semanas (Lunes a Domingo)
            function getWeekDetails(dateStr) {
                const d = new Date((dateStr || toDateKey()) + 'T12:00:00');
                ...
                const fmtHum = dt => pad(dt.getDate()) + '/' + pad(dt.getMonth()+1);

                const startKey = fmt(mon);
                const endKey = fmt(sun);
                return {
                    id: startKey + '_' + endKey,
                    startKey,
                    endKey,
                    label: 'Semana ' + fmtHum(mon) + ' al ' + fmtHum(sun) + ' (' + mon.getFullYear() + ')'
                };
            }

            const currentWeek = getWeekDetails(todayStr) || { id: 'current', label: 'Semana Actual', startKey: todayStr, endKey: todayStr };
            if (!S.accWeekFilter) S.accWeekFilter = currentWeek.id;
            const selectedWeekId = S.accWeekFilter;

            const weeksMap = new Map();
            allHistoricalActive.forEach(s => {
                const d = toDateKey(s.created_at);
                if (d) {
                    const wInfo = getWeekDetails(d);
                    if (wInfo) {
                        if (!weeksMap.has(wInfo.id)) {
                            weeksMap.set(wInfo.id, { ...wInfo, sales: [] });
                        }
                        weeksMap.get(wInfo.id).sales.push(s);
                    }
                }
            });

            // Asegurar que la semana actual siempre esté disponible
            if (!weeksMap.has(currentWeek.id)) {
                weeksMap.set(currentWeek.id, { ...currentWeek, sales: [] });
            }

            const sortedWeeks = Array.from(weeksMap.values()).sort((a,b) => b.startKey.localeCompare(a.startKey));

            const isAllHistory = (selectedWeekId === "all");
            const weekObj = weeksMap.get(selectedWeekId) || currentWeek;
            const weekSales = isAllHistory ? allHistoricalActive : (weekObj.sales || []);

            const weekTotal = weekSales.reduce((acc,s) => acc + Number(s.total||0), 0);
            const weekCashSales = weekSales.filter(s => (s.payment_method || "cash") === "cash");
            const weekCardSales = weekSales.filter(s => s.payment_method === "card");
            const weekCash = weekCashSales.reduce((a,s)=>a+Number(s.total||0), 0);
            const weekCard = weekCardSales.reduce((a,s)=>a+Number(s.total||0), 0);
            const weekTickets = weekSales.length;
            const weekDailyAvg = weekTotal / 7;

            const activeWeekLabel = isAllHistory ? "🌐 Histórico Total Completo" : (weekObj.id === currentWeek.id ? ("🟢 " + weekObj.label + " (Actual)") : ("📆 " + weekObj.label));

            return `
            <div class="dashboard-card" style="background:linear-gradient(135deg,#38050c 0%,#7a0c1c 50%,#44060e 100%);color:#ffffff;border:2.5px solid var(--gold-400);padding:24px;border-radius:20px;margin-bottom:22px;box-shadow:0 8px 26px rgba(0,0,0,0.35)">
                <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:14px;margin-bottom:16px;border-bottom:1.5px solid rgba(254,240,138,0.25);padding-bottom:14px">
                    <div>
                        <span style="color:#fef08a;font-size:11px;font-weight:900;letter-spacing:1.5px;text-shadow:0 1px 3px rgba(0,0,0,0.5)">👑 CONTROL DIRECTIVO SUPERUSUARIO</span>
                        <h2 style="margin:4px 0 2px;font-size:23px;color:#ffffff;font-weight:900;letter-spacing:0.3px">Acumulado Semanal Histórico de las Sucursales</h2>
                        <small style="color:#fde68a;font-weight:800;font-size:12.5px">${activeWeekLabel} • 6 Sucursales Consolidadas</small>
                    </div>
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                        <div style="display:flex;align-items:center;gap:6px">
                            <label style="font-size:12px;font-weight:900;color:#fef08a">SEMANA:</label>
                            <select id="sel-acc-week-filter" style="padding:8px 14px;border-radius:10px;border:2px solid var(--gold-400);font-weight:900;font-size:12.5px;background:#ffffff;color:#1a0205;outline:none;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.2)">
                                <option value="${currentWeek.id}"${selectedWeekId===currentWeek.id?' selected':''}>🟢 Esta Semana (${currentWeek.label})</option>
                                ${sortedWeeks.filter(w => w.id !== currentWeek.id).map(w => `<option value="${w.id}"${selectedWeekId===w.id?' selected':''}>📆 ${w.label}</option>`).join("")}
                                <option value="all"${selectedWeekId==='all'?' selected':''}>🌐 Todo el Histórico Global (${money(grandHistoricalTotal)})</option>
                            </select>
                        </div>
                        <div style="text-align:right;background:rgba(0,0,0,0.25);padding:8px 16px;border-radius:12px;border:1px solid rgba(254,240,138,0.3)">
                            <span style="font-size:11px;color:#fde68a;font-weight:800;display:block">TOTAL SEMANAL</span>
                            <div style="font-size:28px;font-weight:900;color:#ffffff;line-height:1.1">${money(weekTotal)}</div>
                        </div>
                    </div>
                </div>

                <!-- TARJETAS DE TOTALES SEMANALES -->
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:18px">
                    <div style="background:rgba(255,255,255,0.12);padding:12px 16px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.25);box-shadow:0 2px 6px rgba(0,0,0,0.15)">
                        <small style="color:#fde68a;font-size:11px;font-weight:900;display:block;letter-spacing:0.5px">🎟️ TICKETS DE LA SEMANA</small>
                        <strong style="font-size:22px;color:#ffffff;display:block;margin-top:2px">${weekTickets} <small style="font-size:13px">órdenes</small></strong>
                    </div>
                    <div style="background:rgba(22,101,52,0.35);padding:12px 16px;border-radius:12px;border:1.5px solid rgba(134,239,172,0.5);box-shadow:0 2px 6px rgba(0,0,0,0.15)">
                        <small style="color:#86efac;font-size:11px;font-weight:900;display:block;letter-spacing:0.5px">💵 EFECTIVO SEMANAL</small>
                        <strong style="font-size:22px;color:#86efac;display:block;margin-top:2px">${money(weekCash)}</strong>
                        <small style="color:#86efac;font-size:10.5px;font-weight:700">${weekCashSales.length} cobros en caja</small>
                    </div>
                    <div style="background:rgba(30,64,175,0.35);padding:12px 16px;border-radius:12px;border:1.5px solid rgba(147,197,253,0.5);box-shadow:0 2px 6px rgba(0,0,0,0.15)">
                        <small style="color:#93c5fd;font-size:11px;font-weight:900;display:block;letter-spacing:0.5px">💳 TARJETA SEMANAL</small>
                        <strong style="font-size:22px;color:#93c5fd;display:block;margin-top:2px">${money(weekCard)}</strong>
                        <small style="color:#93c5fd;font-size:10.5px;font-weight:700">${weekCardSales.length} cobros en terminal</small>
                    </div>
                    <div style="background:rgba(217,119,6,0.3);padding:12px 16px;border-radius:12px;border:1.5px solid rgba(253,230,138,0.4);box-shadow:0 2px 6px rgba(0,0,0,0.15)">
                        <small style="color:#fde68a;font-size:11px;font-weight:900;display:block;letter-spacing:0.5px">📈 PROMEDIO DIARIO</small>
                        <strong style="font-size:22px;color:#fef08a;display:block;margin-top:2px">${money(weekDailyAvg)}</strong>
                        <small style="color:#fde68a;font-size:10.5px;font-weight:700">Por jornada de 7 días</small>
                    </div>
                </div>

                <!-- DESGLOSE SEMANAL POR SUCURSAL (6 SUCURSALES) -->
                <div>
                    <span style="font-size:11.5px;font-weight:900;color:#fde68a;display:block;margin-bottom:10px;letter-spacing:0.5px">
                        🏢 APORTACIÓN SEMANAL POR SUCURSAL (${activeWeekLabel}):
                    </span>
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px">
                        ${BRANCH_NAMES.map(bn => {
                            const bSales = weekSales.filter(s => matchesBranch(s, bn));
                            const bTot = bSales.reduce((a,s)=>a+Number(s.total||0), 0);
                            const bPct = weekTotal > 0 ? Math.round((bTot / weekTotal) * 100) : 0;
                            return `
                            <div style="background:rgba(255,255,255,0.1);border:1px solid rgba(254,240,138,0.25);padding:10px 12px;border-radius:10px">
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
                                    <strong style="color:#ffffff;font-size:12.5px">🍦 ${esc(bn)}</strong>
                                    <span style="font-size:10px;font-weight:900;color:#fde68a;background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px">${bPct}%</span>
                                </div>
                                <div style="font-size:16px;font-weight:900;color:#ffffff">${money(bTot)}</div>
                                <small style="color:#fcebd2;font-size:10.5px">${bSales.length} tickets en la semana</small>
                            </div>`;
                        }).join("")}
                    </div>
                </div>
        ...
        document.getElementById("sel-acc-week-filter")?.addEventListener("change", async (e) => {
            S.accWeekFilter = e.target.value;
